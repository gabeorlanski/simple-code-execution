...
class ExecutionService:
    ...
    def validate_request(request: ExecuteRequest) -> None:
        ...
        if not request.files and any(
            any(arg.endswith(('.py', '.txt', '.sh', '.js', '.java', '.cpp', '.c'))
    ...
    @staticmethod
    async def execute_request(request: ExecuteRequest) -> Tuple[ExecutableResult, Dict[str, any]]:
        """
        ...
        try:
            ...
            execution_id = await limiter.acquire(priority=request.priority)
            ...
            logger.debug("Using executable type", 
                        execution_id=execution_id,
            ...
            try:
                ...
                error_msg = f"No runner found for executable type: {executable_type}"
